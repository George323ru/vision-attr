...
- **Без отдельной авторизации это публичный URL.** Всё, что лежит на диске по `/data/`,
  может скачать любой. В продовом образе Vision Attractor доступ закрыт серверной
  формой входа через Nginx `auth_request`; см. `docs/PRODUCTION_FORM_LOGIN_AUTH.md`.
- **Если данные чувствительные и нужна персональная авторизация** — нужен бэкенд с
  пользовательскими правами доступа. Текущая форма входа подходит только для общего
  доступа по одному набору credentials.
