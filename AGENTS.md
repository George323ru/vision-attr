...
- Справа сверху quick controls: **Снять выбор** (появляется при активном focus и диспатчит `CLICK_EMPTY`), **Связи** (показать/скрыть фоновый слой всех корреляций), **Близость** (фокусный layout корреляционной близости вокруг выбранного L2 без линий) и **L3** (массовое раскрытие/сворачивание).
...
- В режиме **Близость** фоновые и фокусные корреляционные рёбра скрываются: близость показывается расстоянием от выбранного L2 до напрямую коррелирующих L2.
- Фокусные цветные корреляционные рёбра — только при `focus.type === 'correlations'` для L2 и выключенном режиме **Близость**.

## Layout графа

- `useGraphLayout.computeLayout(attractors, { mode, correlations, proximityFocusNodeId })` поддерживает `mode: 'hierarchy' | 'proximity'`.
- `hierarchy` — базовый layout: L1 фиксированы по кругу, L2/L3 держатся на иерархических пружинах.
- `proximity` без выбранного L2 возвращает обычный `hierarchy` layout.
- `proximity` с выбранным L2 оставляет базовую иерархию, но переносит напрямую коррелирующие L2 вокруг выбранного узла. Reinforcing-связи становятся ближе при росте `strength`; conflicting-связи уводятся дальше и на противоположную сторону при росте `strength`. L3 дочерних перенесённых L2 сдвигаются вместе с parent L2, но в режиме **Близость** L3 скрыты на уровне рендера.
...
Корреляции грузятся из `public/data/correlations.json` через `useCorrelationStore`. Формат:
```typescript
{ id: 'c0001', from: 'l2_byt_01', to: 'l2_byt_02',
  type: 'reinforcing', r: 0.2, strength: 0.2 }
```
