...
- Справа сверху quick controls: **Снять выбор** (появляется при активном focus и диспатчит `CLICK_EMPTY`), **Связи** (показать/скрыть фоновый слой всех корреляций, по умолчанию выключен) и **L3** (массовое раскрытие/сворачивание).
