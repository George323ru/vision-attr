...
├── situations_registry.json       # ⭐ единый source of truth: 41 ситуация (id, title, attractorL2, csvAliases)
...
- `id` — стабильный (`s01..s41`). После миграции `markup.id === situationId === registry.id` — `mkXX` упразднён.
